...
## 4. التحقق من backup قبل النقل

بعد إنشاء backup أو قبل الاعتماد عليه للاستعادة، نفّذ فحصاً سريعاً:

```bash
BACKUP_DIR=~/nile-key-backups/nile-key.test \
./scripts/verify-backup.sh
```

يجب أن ينجح الفحص على الأقل في وجود ملف قاعدة بيانات غير فارغ:

```text
*.sql.gz
```

وإذا كان backup يحتوي على ملفات، يجب أن يحتوي أيضاً على:
